        ...
        necessaryTeamMemberDatas.map((necessaryTeamMemberData) => {
          if (necessaryTeamMemberData.id === itemdata.venderUid) {
            if (itemdata.label === "商談中") {
              necessaryTeamMemberData.inprogressItemsTotalValue += 1;
            }
            if (itemdata.label === "獲得") {
              necessaryTeamMemberData.completedItemsTotalValue += 1;
            }
            if (itemdata.label === "失注") {
              necessaryTeamMemberData.lostOrderItemsTotalValue += 1;
            }
          }
        });
      });
      ...
      {completedData.length && inprogressData.length && memberNames.length && (
        <BarChart
          ...
          series={[
            ...
            { data: lostOrderData, label: "失注", id: "avId", stack: "total" },
          ]}
